package principal;

import java.awt.Image;
import java.io.IOException;
import java.util.ArrayList;
import javax.swing.DefaultListModel;
import javax.swing.Icon;
import javax.swing.ImageIcon;
import javax.swing.JOptionPane;
import logica.Jugador;
import logica.Observable;
import logica.Observer;
import logica.Reglamento;
import logica_sockets.Cliente;
import utileria.Ficha;

public class frmCliente extends javax.swing.JFrame implements Observer {

    private Cliente cliente;
    private Reglamento juego;
    private int[] resultados;
    private Icon canas;

    public frmCliente() {
        initComponents();
        canas = new ImageIcon(new ImageIcon("src\\imgs\\canas.jpeg").getImage().getScaledInstance(100, 65, Image.SCALE_SMOOTH));

        this.btnMover.setEnabled(false);
        this.btnTirar.setEnabled(false);
        this.cmbFichas.setEnabled(false);
        this.setLocationRelativeTo(null);
    }

    /**
     * This method is called from within the constructor to initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is always
     * regenerated by the Form Editor.
     */
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        pnlPrincipal = new javax.swing.JPanel();
        btnTirar = new javax.swing.JButton();
        cmbFichas = new javax.swing.JComboBox<>();
        lblFichas = new javax.swing.JLabel();
        btnMover = new javax.swing.JButton();
        jScrollPane1 = new javax.swing.JScrollPane();
        lstJugadores = new javax.swing.JList<>();
        lblNumJugador2 = new javax.swing.JLabel();
        jPanel1 = new javax.swing.JPanel();
        lblTurno = new javax.swing.JLabel();
        lblNumJugador = new javax.swing.JLabel();
        lblDinero = new javax.swing.JLabel();
        lblResultados = new javax.swing.JLabel();
        lblPorcentaje = new javax.swing.JLabel();
        mnuPrincipal = new javax.swing.JMenuBar();
        mnuConexion = new javax.swing.JMenu();
        mnuConectarse = new javax.swing.JMenuItem();

        setDefaultCloseOperation(javax.swing.WindowConstants.EXIT_ON_CLOSE);
        addWindowListener(new java.awt.event.WindowAdapter() {
            public void windowClosed(java.awt.event.WindowEvent evt) {
                formWindowClosed(evt);
            }
        });

        pnlPrincipal.setBackground(new java.awt.Color(255, 255, 255));
        pnlPrincipal.setBorder(javax.swing.BorderFactory.createEtchedBorder());

        javax.swing.GroupLayout pnlPrincipalLayout = new javax.swing.GroupLayout(pnlPrincipal);
        pnlPrincipal.setLayout(pnlPrincipalLayout);
        pnlPrincipalLayout.setHorizontalGroup(
            pnlPrincipalLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGap(0, 600, Short.MAX_VALUE)
        );
        pnlPrincipalLayout.setVerticalGroup(
            pnlPrincipalLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGap(0, 500, Short.MAX_VALUE)
        );

        btnTirar.setFont(new java.awt.Font("Arial", 0, 18)); // NOI18N
        btnTirar.setText("Tirar cañas");
        btnTirar.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnTirarActionPerformed(evt);
            }
        });

        cmbFichas.setFont(new java.awt.Font("Arial", 0, 18)); // NOI18N

        lblFichas.setFont(new java.awt.Font("Arial", 0, 18)); // NOI18N
        lblFichas.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        lblFichas.setText("# Fichas disponibles");

        btnMover.setFont(new java.awt.Font("Arial", 0, 18)); // NOI18N
        btnMover.setText("Mover ficha");
        btnMover.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                btnMoverActionPerformed(evt);
            }
        });

        lstJugadores.setFont(new java.awt.Font("Arial", 0, 18)); // NOI18N
        jScrollPane1.setViewportView(lstJugadores);

        lblNumJugador2.setFont(new java.awt.Font("Arial", 0, 18)); // NOI18N
        lblNumJugador2.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        lblNumJugador2.setText("Jugadores");

        jPanel1.setBackground(new java.awt.Color(255, 255, 255));
        jPanel1.setBorder(javax.swing.BorderFactory.createEtchedBorder());

        lblTurno.setFont(new java.awt.Font("Arial", 0, 18)); // NOI18N
        lblTurno.setText("Turno del jugador: #");

        lblNumJugador.setFont(new java.awt.Font("Arial", 0, 18)); // NOI18N
        lblNumJugador.setText("Eres el jugador: #");

        lblDinero.setFont(new java.awt.Font("Arial", 0, 18)); // NOI18N
        lblDinero.setText("Tu dinero actual:");

        lblResultados.setFont(new java.awt.Font("Arial", 0, 18)); // NOI18N
        lblResultados.setText("Puntos de la caña:");

        lblPorcentaje.setFont(new java.awt.Font("Arial", 0, 18)); // NOI18N
        lblPorcentaje.setText("Tasa de apuesta:");

        javax.swing.GroupLayout jPanel1Layout = new javax.swing.GroupLayout(jPanel1);
        jPanel1.setLayout(jPanel1Layout);
        jPanel1Layout.setHorizontalGroup(
            jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(jPanel1Layout.createSequentialGroup()
                .addContainerGap()
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(lblTurno)
                    .addComponent(lblNumJugador))
                .addGap(51, 51, 51)
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(lblDinero)
                    .addComponent(lblResultados))
                .addGap(55, 55, 55)
                .addComponent(lblPorcentaje)
                .addContainerGap(javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
        );
        jPanel1Layout.setVerticalGroup(
            jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(jPanel1Layout.createSequentialGroup()
                .addContainerGap()
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(lblTurno)
                    .addComponent(lblDinero)
                    .addComponent(lblPorcentaje))
                .addGap(18, 18, 18)
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(lblNumJugador)
                    .addComponent(lblResultados))
                .addContainerGap(12, Short.MAX_VALUE))
        );

        mnuConexion.setText("Conexión");

        mnuConectarse.setText("Conectarse");
        mnuConectarse.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                mnuConectarseActionPerformed(evt);
            }
        });
        mnuConexion.add(mnuConectarse);

        mnuPrincipal.add(mnuConexion);

        setJMenuBar(mnuPrincipal);

        javax.swing.GroupLayout layout = new javax.swing.GroupLayout(getContentPane());
        getContentPane().setLayout(layout);
        layout.setHorizontalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(layout.createSequentialGroup()
                .addContainerGap()
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(jPanel1, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                    .addGroup(layout.createSequentialGroup()
                        .addComponent(pnlPrincipal, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addComponent(btnTirar, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                            .addComponent(cmbFichas, 0, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                            .addComponent(lblFichas, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                            .addComponent(btnMover, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                            .addComponent(jScrollPane1)
                            .addComponent(lblNumJugador2, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))))
                .addContainerGap())
        );
        layout.setVerticalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, layout.createSequentialGroup()
                .addContainerGap()
                .addComponent(jPanel1, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(layout.createSequentialGroup()
                        .addComponent(btnTirar)
                        .addGap(9, 9, 9)
                        .addComponent(lblFichas)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(cmbFichas, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addGap(18, 18, 18)
                        .addComponent(btnMover)
                        .addGap(18, 18, 18)
                        .addComponent(lblNumJugador2)
                        .addGap(21, 21, 21)
                        .addComponent(jScrollPane1, javax.swing.GroupLayout.PREFERRED_SIZE, 102, javax.swing.GroupLayout.PREFERRED_SIZE))
                    .addComponent(pnlPrincipal, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE))
                .addContainerGap())
        );

        pack();
    }// </editor-fold>//GEN-END:initComponents

    private void formWindowClosed(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowClosed
        if (cliente != null) {
            cliente.cerrar();
            cliente = null;
        }
        this.dispose();
    }//GEN-LAST:event_formWindowClosed

    private void btnTirarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnTirarActionPerformed
        resultados = juego.getDado().tirarDados();
        if (resultados[0] == 0) {
            JOptionPane.showMessageDialog(this, "Sacaste 5 lisas no avanzas", "Info", JOptionPane.INFORMATION_MESSAGE, canas);

            try {
                cliente.actualizarJuego(resultados, Integer.parseInt((String) this.cmbFichas.getSelectedItem()));
            } catch (IOException ex) {
                System.out.println(ex.getMessage() + " error al actualizar el juego desde el cliente");
            }
            this.cmbFichas.setEnabled(false);
        } else {
            JOptionPane.showMessageDialog(this, "Sacaste:" + resultados[0] + " puntos,"
                    + " selecciona una ficha a mover", "Info", JOptionPane.INFORMATION_MESSAGE, canas);
            this.btnMover.setEnabled(true);
            this.cmbFichas.setEnabled(true);
        }
        this.btnTirar.setEnabled(false);
    }//GEN-LAST:event_btnTirarActionPerformed

    private void btnMoverActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnMoverActionPerformed

        try {
            cliente.actualizarJuego(resultados, Integer.parseInt((String) this.cmbFichas.getSelectedItem()));
        } catch (IOException ex) {
            System.out.println(ex.getMessage() + " 2do error al actualizar el juego desde el cliente");
        }

        this.cmbFichas.setEnabled(false);
        this.btnMover.setEnabled(false);
    }//GEN-LAST:event_btnMoverActionPerformed

    private void mnuConectarseActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_mnuConectarseActionPerformed
        String HOST = JOptionPane.showInputDialog(this, "Escriba la IP");
        String puertoStr = JOptionPane.showInputDialog(this, "Escriba el puerto");
        String nombre = JOptionPane.showInputDialog(this, "Ingrese su nombre");
        if (HOST == null || puertoStr == null || nombre == null) {
            JOptionPane.showMessageDialog(this, "No ingrese datos vacios", "Error!", 0);
            return;
        }
        if (HOST.equals("") || puertoStr.equals("") || nombre.equals("")) {
            JOptionPane.showMessageDialog(this, "No ingrese datos vacios", "Error!", 0);
            return;
        }
        int puerto = 0;
        try {
            puerto = Integer.parseInt(puertoStr);
        } catch (NumberFormatException ex) {
            JOptionPane.showMessageDialog(this, "Ingrese un número de puerto valido por favor", "Error!", 0);
            return;
        }

        try {
            cliente = new Cliente(puerto, nombre, HOST);
            cliente.agregarObservador(this);
            Thread hilo = new Thread(cliente);
            hilo.start();
        } catch (IOException ex) {
            JOptionPane.showMessageDialog(this, "Datos de conexión incorrectos, ingreselos de nuevo", "Error!", 0);
        }
    }//GEN-LAST:event_mnuConectarseActionPerformed

    /**
     * @param args the command line arguments
     */
    public static void main(String args[]) {
        /* Set the Nimbus look and feel */
        //<editor-fold defaultstate="collapsed" desc=" Look and feel setting code (optional) ">
        /* If Nimbus (introduced in Java SE 6) is not available, stay with the default look and feel.
         * For details see http://download.oracle.com/javase/tutorial/uiswing/lookandfeel/plaf.html 
         */
        try {
            for (javax.swing.UIManager.LookAndFeelInfo info : javax.swing.UIManager.getInstalledLookAndFeels()) {
                if ("Windows".equals(info.getName())) {
                    javax.swing.UIManager.setLookAndFeel(info.getClassName());
                    break;
                }
            }
        } catch (ClassNotFoundException ex) {
            java.util.logging.Logger.getLogger(frmCliente.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        } catch (InstantiationException ex) {
            java.util.logging.Logger.getLogger(frmCliente.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        } catch (IllegalAccessException ex) {
            java.util.logging.Logger.getLogger(frmCliente.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        } catch (javax.swing.UnsupportedLookAndFeelException ex) {
            java.util.logging.Logger.getLogger(frmCliente.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        }
        //</editor-fold>

        /* Create and display the form */
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new frmCliente().setVisible(true);
            }
        });
    }

    @Override
    public void actualizar(Observable o, Object arg) {
        Cliente clienteSocket = (Cliente) o;
        juego = clienteSocket.getJuego();

        if (juego.isFin()) {
            JOptionPane.showMessageDialog(this, "Ha ganado el jugador: " + juego.getJugadores().get(juego.getGanador()).getNombre(),
                    "Ganador!", JOptionPane.INFORMATION_MESSAGE);
        }

        if (clienteSocket.isIniciado()) {
            iniciarCombo();
        }

        if (clienteSocket.isIniciado()) {
            listarJugadores();
        }

        if (!juego.isFin()) {
            this.lblDinero.setText("Tu dinero actual: " + juego.getJugadores().get(clienteSocket.getNumJugador()).getDinero());
            this.lblNumJugador.setText("Eres el jugador: #" + clienteSocket.getNumJugador());
            this.lblPorcentaje.setText("Tasa de apuesta: " + juego.getManejoApuestas().getPorcentaje());
            this.lblTurno.setText("Turno del jugador: #" + juego.getTurno());
            if (juego.getResultados() != null) {
                this.lblResultados.setText("Puntos de la caña: " + juego.getResultados()[0]);
            }
        }

        pnlPrincipal.removeAll();
        pnlPrincipal.add(juego.getTablero());
        pnlPrincipal.revalidate();
        pnlPrincipal.repaint();

        if ((boolean) arg) {
            JOptionPane.showMessageDialog(this, "Es tu turno de jugar", "!!!", JOptionPane.INFORMATION_MESSAGE);
            this.btnTirar.setEnabled(true);
        }

    }

    private void listarJugadores() {
        DefaultListModel model = new DefaultListModel();
        model.removeAllElements();

        for (Jugador player : this.juego.getJugadores()) {
            model.addElement(player.getNumJugador() + ": " + player.getNombre());
        }

        this.lstJugadores.setModel(model);
    }

    private void iniciarCombo() {
        ArrayList<Ficha> fichas = cliente.getFichas();
        this.cmbFichas.removeAllItems();

        for (Ficha ficha : fichas) {
            if (!ficha.isComida() && !ficha.isMetaFicha()) {
                this.cmbFichas.addItem(ficha.getNumFicha() + "");
            }
        }
    }

    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javax.swing.JButton btnMover;
    private javax.swing.JButton btnTirar;
    private javax.swing.JComboBox<String> cmbFichas;
    private javax.swing.JPanel jPanel1;
    private javax.swing.JScrollPane jScrollPane1;
    private javax.swing.JLabel lblDinero;
    private javax.swing.JLabel lblFichas;
    private javax.swing.JLabel lblNumJugador;
    private javax.swing.JLabel lblNumJugador2;
    private javax.swing.JLabel lblPorcentaje;
    private javax.swing.JLabel lblResultados;
    private javax.swing.JLabel lblTurno;
    private javax.swing.JList<String> lstJugadores;
    private javax.swing.JMenuItem mnuConectarse;
    private javax.swing.JMenu mnuConexion;
    private javax.swing.JMenuBar mnuPrincipal;
    private javax.swing.JPanel pnlPrincipal;
    // End of variables declaration//GEN-END:variables
}
